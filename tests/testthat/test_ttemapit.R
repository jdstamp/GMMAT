...
# 	simdata <- coxed::sim.survdata(N=n_samples, T=100, num.data.frames=1, X=pheno[c("age", "sex")])
# 	survival_time <-simdata$data$y
# 	event_indicator <- as.numeric(simdata$data$failed)
# 	n_missing <- 20
# 	pheno$id <- 1:n_samples
# 	pheno$tte <- survival_time
# 	pheno$disease <- event_indicator
